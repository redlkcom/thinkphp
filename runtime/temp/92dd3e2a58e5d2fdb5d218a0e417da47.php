<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:71:"E:\webserver\PHPTutorial\WWW\tp5\public/../app/wp\view\Index\index.html";i:1562643350;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no">
    <title>京东商城-首页</title>
    <base href="/public/static/wp/">
    <meta name="keywords" content="">
    <meta name="description" content="">
    <script src="js/rem.js"></script>    
    <link href="iconfont/iconfont.css" rel="stylesheet">
    <link href="css/mui.min.css" rel="stylesheet">
    <link href="css/all.css" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="css/base.css"/>
    <link rel="stylesheet" type="text/css" href="css/style.css"/>
</head>
<body>
    <header class="mui-bar mui-bar-nav" id="header">
        <div class="top-sch-box flex-col">
            <div class="centerflex">
                <i class="fdj iconfont icon-search"></i>
                <div class="sch-txt">最新数码设备</div>
                <span class="shaomiao"><i class="iconfont icon-saoma"></i></span>
            </div>
        </div>
        <a class="btn" href="javascript:void(0)">
            <i class="iconfont icon-tixing"></i>
        </a>
        <a class="btn" href="<?php echo url('Cart/index'); ?>">
            <i class="iconfont icon-cart"></i>
        </a>
    </header>

<div id="main" class="mui-clearfix">
	 <!-- 搜索层 -->
    <div class="pop-schwrap">
        <div class="ui-scrollview">
            <div class="mui-bar mui-bar-nav clone">
                <a class="btn btn-back" href="javascript:;"></a>
                <div class="top-sch-box flex-col">
                    <div class="centerflex">
                        <input class="sch-input mui-input-clear" type="text" name="" id="" placeholder="最新数码设备" />
                    </div>
                </div>
                <a class="mui-btn mui-btn-primary sch-submit" href="search.html">搜索</a>
            </div>
            <div class="scroll-wrap">
                <div class="mui-scroll">
                    <div class="sch-cont">
                        <div class="section ui-border-b">
                            <div class="tit"><i class="iconfont icon-hot"></i>热门搜索</div>
                            <div class="tags">
                                <span class="tag">外套</span><span class="tag">连衣裙</span><span class="tag">运动鞋</span><span class="tag">睡衣</span>
                                <span class="tag actice">外套</span><span class="tag">连衣裙</span><span class="tag">运动鞋</span><span class="tag">睡衣</span>
                                <span class="tag">外套</span><span class="tag actice">连衣裙</span><span class="tag">运动鞋</span>
                            </div>
                        </div>
                        <div class="section">
                            <div class="tit"><i class="iconfont icon-time"></i>最近搜索</div>
                            <div class="tags">
                                <span class="tag">外套</span><span class="tag">连衣裙</span><span class="tag">运动鞋</span><span class="tag">睡衣</span>
                                <span class="tag">外套</span><span class="tag">连衣裙</span><span class="tag">运动鞋</span><span class="tag">睡衣</span>
                                <span class="tag">外套</span><span class="tag">连衣裙</span><span class="tag">运动鞋</span>
                            </div>
                        </div>
                    </div>
                    <div class="sch-clear">
                        <a href="javascript:void"><i class="iconfont icon-shanchu"></i>清除搜索历史</a>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="mui-content">
        <div class="banner swiper-container">
            <div class="swiper-wrapper">
                <?php if(is_array($ban_list) || $ban_list instanceof \think\Collection || $ban_list instanceof \think\Paginator): $i = 0; $__LIST__ = $ban_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <div class="swiper-slide"><a href="javascript:void(0)"><img class="swiper-lazy" data-src="/public/Uploads/<?php echo $vo['img']; ?>" alt=""></a></div>
                <?php endforeach; endif; else: echo "" ;endif; ?>
                
            </div>
            <div class="swiper-pagination"></div>
        </div>

        <div class="home-nav ui-box">
            <div class="ui-flex flex-justify-sb">
                <div><a href="javascript:void(0)"><img src="img/homenav1.png" alt="" /></a></div>
                <div><a href="javascript:void(0)"><img src="img/homenav2.png" alt="" /></a></div>
                <div><a href="javascript:void(0)"><img src="img/homenav3.png" alt="" /></a></div>
                <div><a href="javascript:void(0)"><img src="img/homenav4.png" alt="" /></a></div>
            </div>
        </div>

        <div class="home-qnav ui-box">
            <div class="ui-flex flex-justify-sb">
                <div><a href="javascript:void(0)"><img src="img/qnav1.png" class="ico" /><span class="name">我的店铺</span></a></div>
                <div><a href="javascript:void(0)"><img src="img/qnav2.png" class="ico" /><span class="name">招商加盟</span></a></div>
                <div><a href="javascript:void(0)"><img src="img/qnav3.png" class="ico" /><span class="name">我的喜欢</span></a></div>
                <div><a href="javascript:void(0)"><img src="img/qnav4.png" class="ico" /><span class="name">猜你喜欢</span></a></div>
            </div>
        </div>

        <div class="home-newgoods ui-box">
            <img class="home-imgtit" src="img/hometit1.jpg" alt="" />
            <div class="list-type1 plist-puzzle">
                <a class="b" href="<?php echo url('Cate/details',['id'=>$vo['id']]); ?>"><img src="/public/Uploads/<?php echo $list_copy['data']['0']['img']['1']; ?>" alt="" /></a>
                <div class="s ui-flex-vt flex-justify-sb">
                    <?php if(is_array($list_copy['data']) || $list_copy['data'] instanceof \think\Collection || $list_copy['data'] instanceof \think\Paginator): $i = 0; $__LIST__ = $list_copy['data'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                    <a class="box" href="<?php echo url('Cate/details',['id'=>$vo['id']]); ?>"><img src="/public/Uploads/<?php echo $vo['img']['0']; ?>" alt="" /></a>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </div>
            </div>
            <img class="home-imgtit" src="img/hometit2.jpg" alt="" />
            <!-- <div class="list-type2 ui-flex flex-justify-sb">
                <a class="box" href=""><img class="figure" src="uploads/t3.jpg" alt="" /><span class="tit">手机</span></a>
                <a class="box" href=""><img class="figure" src="uploads/t3.jpg" alt="" /><span class="tit">家电</span></a>
                <a class="box" href=""><img class="figure" src="uploads/t3.jpg" alt="" /><span class="tit">智能</span></a>
            </div> -->
        </div>

        <div class="home-fashion ui-box ui-border-t">
            <img class="home-imgtit" src="img/hometit3.jpg" alt="" />
            <img class="db margin-b-s" src="/public/Uploads/<?php echo $ban_list['0']['img']; ?>" width="100%" alt="" />
            <div class="fastion-plist mui-row">
                <?php if(is_array($list_copy['data']) || $list_copy['data'] instanceof \think\Collection || $list_copy['data'] instanceof \think\Paginator): $i = 0; $__LIST__ = $list_copy['data'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <div class="mui-col-xs-4">
                    <a class="item" href="<?php echo url('Cate/details',['id'=>$vo['id']]); ?>">
                        <img src="/public/Uploads/<?php echo $vo['img']['0']; ?>" alt="" class="figure" /><span class="tit"><?php echo $vo['name']; ?></span>
                    </a>
                </div>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </div>
        </div>
    </div> <!--mui-content end-->
</div>
   

    <footer class="page-footer fixed-footer" id="footer">
		<ul>
			<li class="active">
				<a href="<?php echo url('Index/index'); ?>">
					<img src="images/footer01.png"/>
					<p>首页</p>
				</a>
			</li>
			<li>
				<a href="<?php echo url('Asort/index'); ?>">
					<img src="images/footer002.png"/>
					<p>分类</p>
				</a>
			</li>
			<li>
				<a href="<?php echo url('Cart/index'); ?>">
					<img src="images/footer003.png"/>
					<p>购物车</p>
				</a>
			</li>
			<li>
				<a href="<?php echo url('Vip/index'); ?>">
					<img src="images/footer004.png"/>
					<p>个人中心</p>
				</a>
			</li>
		</ul>
	</footer>

</body>
<script type="text/javascript" src="js/jquery-1.8.3.min.js" ></script>
<script src="js/fastclick.js"></script>
<script src="js/mui.min.js"></script>
<script type="text/javascript" src="js/hmt.js" ></script>
<!--插件-->
<link rel="stylesheet" href="js/swiper/swiper.min.css">
<script src="js/swiper/swiper.jquery.min.js"></script>
<!--插件-->
<script src="js/global.js"></script>
<script >
    $(function () {
        var banner = new Swiper('.banner',{
            autoplay: 5000,
            pagination : '.swiper-pagination',
            paginationClickable: true,
            lazyLoading : true,
            loop:true
        });

         mui('.pop-schwrap .sch-input').input();
        var deceleration = mui.os.ios?0.003:0.0009;
         mui('.pop-schwrap .scroll-wrap').scroll({
                bounce: true,
                indicators: true,
                deceleration:deceleration
        });
        $('.top-sch-box .fdj,.top-sch-box .sch-txt,.pop-schwrap .btn-back').on('click',function () {
            $('html,body').toggleClass('holding');
            $('.pop-schwrap').toggleClass('on');
            if($('.pop-schwrap').hasClass('on')) {;
                $('.pop-schwrap .sch-input').focus();
            }
        });

    });
</script>
</html>